...
//------------------------------####################################
// Here using address of mal during deployment of foo and calling bar = Bar(_bar) (_bar is address of mal) 
// in constructor means load the interface of Bar contract at address of mal contract, and since the functions
// in both bar and mal contract is same log function of mal contract is executed.
...
contract Foo {
    ...
    function usingCall() public  {
        (bool ok, ) = address(bar).call(abi.encodeWithSignature("log()"));
    }
}
