...
// When Alice is tricked to call Attack.attack() function, the initeatior of transaction is Alice
// so in Wallet.transfer() the tx.origin is alice, which passes the check tx.origin=owner=alice(during constructor)
// in short this thing is working // If contract A calls B, and B calls C, in C msg.sender is B and tx.origin is A.
